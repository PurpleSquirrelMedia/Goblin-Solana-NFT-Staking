...
import { ADMIN_KEY, PROGRAM_ID, PUBLISH_NETWIRK } from '../config';

export const solConnection = new web3.Connection(web3.clusterApiUrl(PUBLISH_NETWIRK));
...
const initAdminKey = new PublicKey(ADMIN_KEY);
...
export const initProject = async (
  ...
  let ix = SystemProgram.createAccount({
    fromPubkey: wallet.publicKey,
    newAccountPubkey: globalLotteryKey,
    lamports : await provider.connection.getMinimumBalanceForRentExemption(GLOBAL_POOL_SIZE),
    space: GLOBAL_POOL_SIZE,
    programId: program.programId,
  })*/
...
export const getLotteryState = async (
  ...
  // const [poolWalletKey] = await PublicKey.findProgramAddress(
  //   [Buffer.from(POOL_WALLET_SEED)],
  //   program.programId
  // );

...
import { INIT_WALLETKEY, PUBLISH_NETWIRK } from '../config';
import mintedNftsList from '../goblin_army_mint_list.json';
import mintedNftsListDev from '../goblin_army_mint_list_devnet.json';
...
export default function Products() {
  ...
  let mintedNfts = PUBLISH_NETWIRK === "mainnet" && mintedNftsList;
  mintedNfts = PUBLISH_NETWIRK === "devnet" && mintedNftsListDev;
  ...
  const getClaimRewardValue = (rewardTime, items) => {
    ...
    setClaimRewardValue(sum + pendingReward)
    setForseRender(!forseRender)
  }
  ...
  const updateFixedPoolState = (addr) => {
    ...
    getFixedState(addr).then(async result => {
      if (result !== null) {
        ...
        setPendingReward(result.pendingReward.toNumber() / LAMPORTS_PER_SOL)
  ...
  const goblinValidate = (mintAddress) => {
    if (mintedNfts.indexOf(mintAddress) !== -1) {
      return true
    } else {
      return false
    }
  ...
  const [claimFix, setclaimFix] = useState(false)
  ...
  return (
    <Page title="Goblin Army | Product, Stake, Unstake, and Claim">
      {wallet.publicKey?.toBase58() === INIT_WALLETKEY &&
        <div style={{ marginBottom: 10, width: "100%", textAlign: "right" }}>
          <Button variant="contained" onClick={() => initProject(wallet)}>
            init project
          </Button>
        </div>
      }
            ...
            <div className="lottery-holders-content">
              ...
              {lotteryHoders.length !== 0 && lotteryHoders.map((item, key) => (
                ...
                <p key={key} className="holder-item">
                  <a href={"https://solscan.io/token/" + item.nftAddr.toBase58() + "?cluster=" + PUBLISH_NETWIRK} target="_blank" rel="noreferrer"><span className="mobile-label">MINT ADDRESS : </span>{item.nftAddr.toBase58().slice(0, 10) + "..." + item.nftAddr.toBase58().slice(34, 44)}</a>
          ...
          {!loading &&
            <div className="nfts-tabs">
              <div className={tab ? "nfts-tab-item active" : "nfts-tab-item"} onClick={() => setTab(true)}>
                Staked NFTs({lotteryState.itemCount + fixedState.itemCount})
              </div>
              <div className={!tab ? "nfts-tab-item active" : "nfts-tab-item"} onClick={() => setTab(false)}>
                Unstaked NFTs({unstakedNftList.length})
              </div>
            </div>
          }
          {loading &&
                      ...
                      } */}
                      {
                        lotteryState.items.map((item, id) => (
                          <NFTCard
                            ...
                            setclaimFix={() => setclaimFix(!claimFix)}
                            key={id}
                          />
                        ))
                      }
